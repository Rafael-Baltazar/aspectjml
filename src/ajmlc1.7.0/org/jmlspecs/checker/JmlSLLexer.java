// $ANTLR 2.7.7 (20070227): "expandedJmlSL.g" -> "JmlSLLexer.java"$
 ...
 *
 * $Id: JmlSL.g,v 1.14 2005/06/09 10:42:47 davidcok Exp $
 */
...
public class JmlSLLexer extends antlr.CharScanner implements JmlSLLexerTokenTypes, TokenStream
 {

    public JmlSLLexer(ParsingController parsingController,
...
public JmlSLLexer(InputStream in) {
	this(new ByteBuffer(in));
}
public JmlSLLexer(Reader in) {
	this(new CharBuffer(in));
}
public JmlSLLexer(InputBuffer ib) {
	this(new LexerSharedInputState(ib));
}
public JmlSLLexer(LexerSharedInputState state) {
	super(state);
...
tryAgain:
	...
	protected final void mML_COMMENT_REST(boolean _createToken) throws RecognitionException, CharStreamException, TokenStreamException {
		...
		if ((_tokenSet_1.member(LA(1)))) {
			...
			_loop12:
			do {
				if (((LA(1)=='*') && (_tokenSet_2.member(LA(2))) && (_tokenSet_2.member(LA(3))))&&( LA(2)!='/' )) {
					match('*');
				}
			...
			if ( inputState.guessing==0 ) {
						...
						throw new TokenStreamException( 
						    new Message( JmlMessages.NESTED_ANNOTATION ).getMessage()
						);
	...
	public final void mINFORMAL_DESC(boolean _createToken) throws RecognitionException, CharStreamException, TokenStreamException {
		...
		_loop22:
		do {
			if (((LA(1)=='*') && (_tokenSet_2.member(LA(2))) && (_tokenSet_2.member(LA(3))) && (true))&&(LA(2) != ')')) {
				_saveIndex=text.length();
				match('*');
				text.setLength(_saveIndex);
			}
...
inputState.guessing--;
	...
	public final void mINTEGER_LITERAL(boolean _createToken) throws RecognitionException, CharStreamException, TokenStreamException {
		...
		case '8':  case '9':
		{
			...
			else if (((_tokenSet_9.member(LA(1))))&&(isDecimal && !(LA(1)=='.' && LA(2)=='.'))) {
				{
				switch ( LA(1)) {
				case '.':
				{
