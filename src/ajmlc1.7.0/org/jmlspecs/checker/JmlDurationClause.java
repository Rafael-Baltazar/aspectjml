 ...
 *
 * $Id: JmlDurationClause.java,v 1.5 2005/09/12 19:02:00 cruby Exp $
 */
...
public class JmlDurationClause extends JmlPredicateClause {
    ...
    // CONSTRUCTORS 
    //---------------------------------------------------------------------
    public JmlDurationClause( TokenReference where,
	...
	// create a new duration expression context 
	...
	check(context, specExp.getType().isOrdinal(),
	      JmlMessages.BAD_TYPE_IN_DURATION_CLAUSE,
	      specExp.getType().toVerboseString() );
	...
	if (p instanceof JmlVisitor)
	    ((JmlVisitor)p).visitJmlDurationClause(this);
	else
