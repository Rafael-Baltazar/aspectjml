// $ANTLR 2.7.7 (20070227): "expandedJmlJD.g" -> "JmlJDLexer.java"$
 ...
 *
 * $Id: JmlJD.g,v 1.14 2005/06/09 10:42:47 davidcok Exp $
 */
...
public class JmlJDLexer extends antlr.CharScanner implements JmlJDLexerTokenTypes, TokenStream
 {
    ...
    /**
     * Find the keyword literal token for the given buffer.  This
     * method is modified in the lexers of grammars that inherit
     * from this to search different keyword sets.  */
    private Token keywordLookup( char[] cache, int start, int len ) {
        return JmlIDKeywords.lookup( cache, start, len );
    }
...
public JmlJDLexer(InputBuffer ib) {
	this(new LexerSharedInputState(ib));
}
public JmlJDLexer(LexerSharedInputState state) {
	super(state);
...
inputState.guessing--;
	...
	public final void mEMBEDDED_JML_CLOSE(boolean _createToken) throws RecognitionException, CharStreamException, TokenStreamException {
		int _ttype; Token _token=null; int _begin=text.length();
		_ttype = EMBEDDED_JML_CLOSE;
		int _saveIndex;
...
inputState.guessing--;
	...
	public final void mINTEGER_LITERAL(boolean _createToken) throws RecognitionException, CharStreamException, TokenStreamException {
		...
		case '8':  case '9':
		{
			...
			else if (((_tokenSet_13.member(LA(1))))&&(isDecimal && !(LA(1)=='.' && LA(2)=='.'))) {
				{
				switch ( LA(1)) {
				case '.':
				{
	...
	protected final void mEXPONENT(boolean _createToken) throws RecognitionException, CharStreamException, TokenStreamException {
		...
		do {
			if (((LA(1) >= '0' && LA(1) <= '9'))) {
				...
				if ( _cnt186>=1 ) { break _loop186; } else {throw new NoViableAltForCharException((char)LA(1), getFilename(), getLine(), getColumn());}
			}
	...
	private static final long[] mk_tokenSet_1() {
		long[] data = { 4402341484032L, 0L, 0L, 0L, 0L};
		return data;
	}
