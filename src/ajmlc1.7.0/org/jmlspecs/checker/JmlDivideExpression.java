 ...
 *
 * $Id: JmlDivideExpression.java,v 1.7 2004/06/23 19:43:05 f_rioux Exp $
 */
 ...
 */
public class JmlDivideExpression extends JDivideExpression {
     ...
     */
    public JmlDivideExpression( TokenReference where,
			   JExpression left,
			   JExpression right ) {
    ...
    public JExpression verifyOperation(JExpression expr,  CExpressionContextType context ){
    	...
    	return JmlBinaryArithmeticExpressionHelper.verifyOperation(expr, context, OPE_SLASH);
    }

    /**
     * Override of super class method to prevent constant folding (since in
     * general we might not be able to represent as a literal the quotient
	...
	public int compute(int left, int right, CExpressionContextType context) {
		...
		JmlBinaryArithmeticExpressionHelper.compute(this, left, right, context, OPE_SLASH);
