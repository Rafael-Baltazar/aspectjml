// $ANTLR 2.7.7 (20070227): "expandedJml.g" -> "JmlLexer.java"$
...
import antlr.collections.impl.BitSet;
    ...
    /**
	 * This is the top level JML scanner.  It processes the normal
	 * (Multi)Java lexicon and switches to annotation lexers for
	 * processing the additional keywords that can appear inside
	 * annotations. */

public class JmlLexer extends antlr.CharScanner implements JmlLexerTokenTypes, TokenStream
 {

    public JmlLexer(ParsingController parsingController,
	...
	case JmlTokenTypes.LITERAL_non_null:
	case JmlTokenTypes.LITERAL_nullable:
	    if( !aboutToProcessABoundVariable ) {
		result = null;
	    }
	    break;
	 ...
	 * resolving IDENTs.  This field is false by default in the top-level
     * lexer and true by default in the multi-line and single-line annotation
     * lexers that inherit from this.
     */
    private boolean inAnnotation = false;

    /** Called to specify that the whole file is an annotation, that is, it is
	    a jml file rather than a spec or java file.
	*/
    public void inAnnotation() { inAnnotation = true; }

    /**
     * 
     */
    public boolean aboutToProcessABoundVariable = false;
	 ...
	 * </jml></pre>
	 * */
    private final boolean allowAnnotations = true;
...
inputState.guessing--;
	...
	public final void mJAVADOC_OPEN(boolean _createToken) throws RecognitionException, CharStreamException, TokenStreamException {
		...
		if (!( LA(4)!='/' ))
		  throw new SemanticException(" LA(4)!='/' ");
		match("/**");
		if ( inputState.guessing==0 ) {
			
				    try {
					parsingController.push("javadoc");
				    } catch( ParsingController.KeyException ke ) {
					throw
					new TokenStreamException( "No javadoc lexer available." );
				    }
				    _token =  ParsingController.IGNORE;
	...
	protected final void mNOWARN_LABEL(boolean _createToken) throws RecognitionException, CharStreamException, TokenStreamException {
		...
		do {
			...
			default:
			{
				if ( _cnt715>=1 ) { break _loop715; } else {throw new NoViableAltForCharException((char)LA(1), getFilename(), getLine(), getColumn());}
			}
			}
			_cnt715++;
		} while (true);
	...
	public final void mINTEGER_LITERAL(boolean _createToken) throws RecognitionException, CharStreamException, TokenStreamException {
			...
			if (((LA(1) >= '0' && LA(1) <= '9'))) {
				...
				do {
					if (((LA(1) >= '0' && LA(1) <= '9'))) {
						...
						if ( _cnt723>=1 ) { break _loop723; } else {throw new NoViableAltForCharException((char)LA(1), getFilename(), getLine(), getColumn());}
					}
		...
		case '8':  case '9':
		{
				...
				case 'X':  case 'x':
				{
					...
					do {
						if ((_tokenSet_11.member(LA(1))) && (true) && (true) && (true)) {
							...
							if ( _cnt730>=1 ) { break _loop730; } else {throw new NoViableAltForCharException((char)LA(1), getFilename(), getLine(), getColumn());}
						}
			...
			else if (((_tokenSet_12.member(LA(1))))&&(isDecimal && !(LA(1)=='.' && LA(2)=='.'))) {
				{
				switch ( LA(1)) {
				case '.':
				{
	...
	public final void mML_EMPTY_COMMENT(boolean _createToken) throws RecognitionException, CharStreamException, TokenStreamException {
		int _ttype; Token _token=null; int _begin=text.length();
		...
		case 'u':
		{
			...
			do {
				if ((LA(1)=='u')) {
					...
					if ( _cnt801>=1 ) { break _loop801; } else {throw new NoViableAltForCharException((char)LA(1), getFilename(), getLine(), getColumn());}
				}
