...
public class InvariantMethodAdviceAsPostconditionMethod extends InvariantLikeMethod{
	...
	protected StringBuffer buildAfterAdvice() 
	{
		...
		if((this.hasInstInv) || (this.hasStatInv)){
			...
			if( (this.hasInstInv) && (this.hasStatInv)){
				code.append("after (final ").append(AspectUtil.replaceDollaSymbol(classQualifiedName)).append(" ").append("object$rac");	
				code.append(")").append(" :");
				code.append("\n");
				code.append("   (execution(!static * "+classQualifiedName+"+.*(..))").append(" || \n");
				if(this.typeDecl.getCClass().isAbstract()){
					code.append("     (execution("+classQualifiedName+"+.new(..))").append(" && !within("+classQualifiedName+"))").append(adviceexecution).append(")").append(" && \n");
				}
				else {
					code.append("     execution("+classQualifiedName+"+.new(..))").append(adviceexecution).append(")").append(" && \n");
				}
				code.append("   !execution(void "+classQualifiedName+".finalize() throws Throwable)");
				code.append(" && ");
				code.append("\n");	
				...
				code.append("   (execution( * "+classQualifiedName+"+.*(..))").append(" || \n");
				if(this.typeDecl.getCClass().isAbstract()){
					code.append("     (execution("+classQualifiedName+"+.new(..))").append(" && !within("+classQualifiedName+"))").append(" || \n");
				}
				else {
					code.append("     execution("+classQualifiedName+"+.new(..))").append(" || \n");
				}
