	...
	protected PreValueVars genAssertionMethods()
	{
		...
		if(methodHasRealExplicitSpecs()){
			...
			while (iter.hasNext()) {
				...
				JmlDataGroupMemberMap tdg= typeDecl.getDataGroupMap();
//				System.out.println("-------------------------");
//				System.out.println(tdg.toString());
//				System.out.println("-------------------------");
//				System.out.println("");
				
				for (Iterator iterator = tdg.keyGroupIterator(); iterator
						.hasNext();) {
					JmlSourceField currentJmlSourceField = (JmlSourceField) iterator.next();
//					System.out.println("key --> "+currentJmlSourceField.getIdent()+" isModel--> "+currentJmlSourceField.isModel());
					JmlAssignableFieldSet fieldSet = tdg.getMembers(currentJmlSourceField);
//					System.out.print(" {");
					for (Iterator iterator2 = fieldSet.iterator(); iterator2
							.hasNext();) {
						CField object = (CField) iterator2.next();
//						System.out.print(object.getJavaName());
//						System.out.print(" ");
					}
//					System.out.print(" }");
//					System.out.println();
...
//				JmlSourceField
				...
				if(c.assignable() != null){
					JmlAssignableClause[] jac = c.assignable();
					for (int i = 0; i < jac.length; i++) {
						JmlAssignableClause currentAssignable = jac[i];
						JmlAssignableFieldSet jafs = currentAssignable.getAssignableFieldSet();
//	                    System.out.println("## ->"+jafs.toString());
	                    
	                    
	                    for (Iterator iterator = jafs.iterator(); iterator
								.hasNext();) {
							CField object = (CField) iterator.next();
//							System.out.println("field = "+object.getJavaName());
//							System.out.println("field type = "+object.getType().toString());
						}
					}
				}
				...
				if(!isMethodCrosscutSpecChecking){
					if (preNonNullExpr != null) {
						...
						if (preNonNullExpr instanceof RacPredicate) ((RacPredicate)preNonNullExpr).countCoverage();
				...
				if (c.hasPrecondition() && pred != null) {
					...
					preStmt = RacParser.parseStatement("",
							preStmt, transPre.stmt(true));
				...
				if (postNonNullExpr != null) {
					...
					if (postNonNullExpr instanceof RacPredicate) ((RacPredicate)postNonNullExpr).countCoverage();
					...
					if (this.hasFieldReferenceInPrecondition(transPreStr) || this.hasMethodCallExpInPrecondition(transPreStr)){
						if(c.hasPostcondition()){
							if(Main.aRacOptions.clientAwareChecking()){
								//verifying visibility for translation and composition (combination of spec cases)
								if(visibility == ACC_PUBLIC){
									publicEpostCode.add("\n\t\t   if ("+hasRacB + AspectUtil.changeThisOrSuperRefToAdviceRef(((String)publicPreExprsDecl.get(publicPreExprsDeclCount-1)),typeDecl) + ") {");
								}
								else if(visibility == ACC_PROTECTED){
									protectedEpostCode.add("\n\t\t   if ("+hasRacB + AspectUtil.changeThisOrSuperRefToAdviceRef(((String)protectedPreExprsDecl.get(protectedPreExprsDeclCount-1)),typeDecl) + ") {");
								}
								else if(visibility == 0L){
									defaultEpostCode.add("\n\t\t   if ("+hasRacB + AspectUtil.changeThisOrSuperRefToAdviceRef(((String)defaultPreExprsDecl.get(defaultPreExprsDeclCount-1)),typeDecl) + ") {");
								}
								else if(visibility == ACC_PRIVATE){
									privateEpostCode.add("\n\t\t   if ("+hasRacB + AspectUtil.changeThisOrSuperRefToAdviceRef(((String)privatePreExprsDecl.get(privatePreExprsDeclCount-1)),typeDecl) + ") {");
								}
							}
							epostCode.add("\n\t\t   if ("+hasRacB + AspectUtil.changeThisOrSuperRefToAdviceRef(((String)preExprsDecl.get(preExprsDeclCount-1)),typeDecl) + ") {");
						}
						else{
							String var = "rac$pre"+preExprsDeclCount;
							preExprsDecl.add(var);
						...
						if(Main.aRacOptions.clientAwareChecking()){
							//verifying visibility for translation and composition (combination of spec cases)
							if(visibility == ACC_PUBLIC){
								publicEpostCode.add("\n\t\t   if ("+hasRacB + AspectUtil.changeThisOrSuperRefToAdviceRef(transPreStr, typeDecl) + ") {");
							}
							else if(visibility == ACC_PROTECTED){
								protectedEpostCode.add("\n\t\t   if ("+hasRacB + AspectUtil.changeThisOrSuperRefToAdviceRef(transPreStr, typeDecl) + ") {");
							}
					...
					for (int i = 0; i < xPostcondition.length; i++) {
						if (xPostcondition[i].hasPredicate() && !xPostcondition[i].isNotSpecified()) {
							...
							for (Iterator iterator = AspectUtil.getInstance().getDefaultSignalsClauseTokenRefereces().iterator(); iterator.hasNext();) {
								...
								if(xPostcondition[i].predOrNot().getTokenReference().toString().equals(tokenReference)){
									isEqual = true;
								}
							}//

							String currentEpostTranslated = translateSignalsClause(postVarGen, 
									preVarGen, xPostcondition[i], xPostconditionForContext, (!isEqual), AspectUtil.changeThisOrSuperRefToAdviceRef(transPreStr, this.typeDecl), -1, index, oldExprs, oldExprsDecl);
							...
							if(Main.aRacOptions.clientAwareChecking()){
								...
								if(visibility == ACC_PUBLIC){
									currentEpostTranslated = translateSignalsClause(postVarGen, 
											preVarGen, xPostcondition[i], publicXPostconditionForContext, (!isEqual), AspectUtil.changeThisOrSuperRefToAdviceRef(transPreStr, this.typeDecl), visibility, index, publicOldExprs, publicOldExprsDecl);
									publicEpostCode.add(currentEpostTranslated);
								}
								else if(visibility == ACC_PROTECTED){
									currentEpostTranslated = translateSignalsClause(postVarGen, 
											preVarGen, xPostcondition[i], protectedXPostconditionForContext, (!isEqual), AspectUtil.changeThisOrSuperRefToAdviceRef(transPreStr, this.typeDecl), visibility, index, protectedOldExprs, protectedOldExprsDecl);
									protectedEpostCode.add(currentEpostTranslated);
								}
								else if(visibility == 0L){
									currentEpostTranslated = translateSignalsClause(postVarGen, 
											preVarGen, xPostcondition[i], defaultXPostconditionForContext, (!isEqual), AspectUtil.changeThisOrSuperRefToAdviceRef(transPreStr, this.typeDecl), visibility, index, defaultOldExprs, defaultOldExprsDecl);
									defaultEpostCode.add(currentEpostTranslated);
								}
								else if(visibility == ACC_PRIVATE){
									currentEpostTranslated = translateSignalsClause(postVarGen, 
											preVarGen, xPostcondition[i], privateXPostconditionForContext, (!isEqual), AspectUtil.changeThisOrSuperRefToAdviceRef(transPreStr, this.typeDecl), visibility, index, privateOldExprs, privateOldExprsDecl);
		...
		// if the current method does not have explicit spec cases
			...
			if (signals_clause != null) {
				...
				if (signals_clause.hasPredicate() && !signals_clause.isNotSpecified()) {
					...
					String currentEpostTranslated = translateSignalsClause(postVarGen, 
							preVarGen, signals_clause, xPostconditionForContext, (!isEqual), "true", -1, 0, oldExprs, oldExprsDecl);
					epostCode.add(currentEpostTranslated);
					...
					if(Main.aRacOptions.clientAwareChecking()){
						...
						if(visibility == ACC_PUBLIC){
							currentEpostTranslated = translateSignalsClause(postVarGen, 
									preVarGen, signals_clause, publicXPostconditionForContext, (!isEqual), "true", visibility, 0, publicOldExprs, publicOldExprsDecl);
							publicEpostCode.add(currentEpostTranslated);
						}
						else if(visibility == ACC_PROTECTED){
							currentEpostTranslated = translateSignalsClause(postVarGen, 
									preVarGen, signals_clause, protectedXPostconditionForContext, (!isEqual), "true", visibility, 0, protectedOldExprs, protectedOldExprsDecl);
							protectedEpostCode.add(currentEpostTranslated);
						}
						else if(visibility == 0L){
							currentEpostTranslated = translateSignalsClause(postVarGen, 
									preVarGen, signals_clause, defaultXPostconditionForContext, (!isEqual), "true", visibility, 0, defaultOldExprs, defaultOldExprsDecl);
							defaultEpostCode.add(currentEpostTranslated);
						}
						else if(visibility == ACC_PRIVATE){
							currentEpostTranslated = translateSignalsClause(postVarGen, 
									preVarGen, signals_clause, privateXPostconditionForContext, (!isEqual), "true", visibility, 0, privateOldExprs, privateOldExprsDecl);
							privateEpostCode.add(currentEpostTranslated);
						}
					}
				}
		...
		// accumlate pre-state variables that need to be saved and
		// restored before calling the original method, due to
		...
		/*if (!(backupVars.isEmpty() ||
				(methodDecl instanceof JmlConstructorDeclaration))) {
			stackVar = varGen.freshStackVar();
			saveMethod = MN_SAVE_TO + stackVar;
			restoreMethod = MN_RESTORE_FROM + stackVar;
		}*/

		//specVars.addAll(oldExprs);

		// generate pre-, and postcondition checking methods advice using Dynamic Crosscutting
		// only call site using CAC technique --- [[[hemr]]]
		if(Main.aRacOptions.clientAwareChecking()){
			if (((AspectUtil.getInstance().hasElementsStoredOldExpressions(publicOldExprs)) || (publicPreExprs.size() > 0) || (AspectUtil.getInstance().hasElementsStoredOldExpressions(publicOldVarsDecl)))) {
				...
				AspectUtil.getInstance().appendPostconditionNewMethodsAroundAdvice(npma.generate(postStmt, publicPrecondition.toString(),
						...
						publicEpostCode, publicOldExprs, publicOldExprsDecl, publicPreExprs,
										publicPreExprsDecl, publicOldVarsDecl, "clientAwareChecking", ACC_PUBLIC, exceptionsInSignalsClauses));
			}
			else{
				PreconditionMethodAdvice pma = new PreconditionMethodAdvice(typeDecl, 
						methodDecl, hasPublicPrecondition, saveMethod);
				if(AspectUtil.hasPrecondition(publicPrecondition.toString())){
					AspectUtil.getInstance().appendPreconditionNewMethodsAdvice(pma.generate(preStmt, publicPrecondition.toString(), this
							.getContextValues(publicPrecondition.toString()),
							publicPreTokenReference.toString(), "clientAwareChecking", ACC_PUBLIC, isEmptySpecCaseWithNonNullPre));
				}
				...
				if(AspectUtil.hasAssertion(publicNPostcondition.toString()) || (publicEpostCode.size() > 0)){
					JMethodDeclarationType jmdt = npma.generate(postStmt,
							AspectUtil.changeThisOrSuperRefToAdviceRef(publicNPostcondition.toString(), this.typeDecl), publicEpostCode,
									publicOldExprs,
									publicOldExprsDecl, publicPreExprs, publicPreExprsDecl, publicOldVarsDecl, "clientAwareChecking", ACC_PUBLIC, exceptionsInSignalsClauses);
			...
			if (((AspectUtil.getInstance().hasElementsStoredOldExpressions(protectedOldExprs)) || (protectedPreExprs.size() > 0) || (AspectUtil.getInstance().hasElementsStoredOldExpressions(protectedOldVarsDecl)))) {
				PostconditionMethodAdvice2 npma = new PostconditionMethodAdvice2(typeDecl, 
						methodDecl, restoreMethod);
				AspectUtil.getInstance().appendPostconditionNewMethodsAroundAdvice(npma.generate(postStmt, protectedPrecondition.toString(),
						...
						protectedEpostCode, protectedOldExprs, protectedOldExprsDecl, protectedPreExprs,
										protectedPreExprsDecl, protectedOldVarsDecl, "clientAwareChecking", ACC_PROTECTED, exceptionsInSignalsClauses));
			}
			else{
				PreconditionMethodAdvice pma = new PreconditionMethodAdvice(typeDecl, 
						methodDecl, hasProtectedPrecondition, saveMethod);
				if(AspectUtil.hasPrecondition(protectedPrecondition.toString())){
					AspectUtil.getInstance().appendPreconditionNewMethodsAdvice(pma.generate(preStmt, protectedPrecondition.toString(), this
							.getContextValues(protectedPrecondition.toString()),
							protectedPreTokenReference.toString(), "clientAwareChecking", ACC_PROTECTED, isEmptySpecCaseWithNonNullPre));
				}
				PostconditionMethodAdvice npma = new PostconditionMethodAdvice(typeDecl, 
						methodDecl, restoreMethod);
				...
				if(AspectUtil.hasAssertion(protectedNPostcondition.toString()) || (protectedEpostCode.size() > 0)){
					JMethodDeclarationType jmdt = npma.generate(postStmt,
							AspectUtil.changeThisOrSuperRefToAdviceRef(protectedNPostcondition.toString(), this.typeDecl), protectedEpostCode, protectedOldExprs,
									protectedOldExprsDecl, protectedPreExprs, protectedPreExprsDecl, protectedOldVarsDecl, "clientAwareChecking", ACC_PROTECTED, exceptionsInSignalsClauses);
			...
			if (((AspectUtil.getInstance().hasElementsStoredOldExpressions(defaultOldExprs)) || (defaultPreExprs.size() > 0) || (AspectUtil.getInstance().hasElementsStoredOldExpressions(defaultOldVarsDecl)))) {
				PostconditionMethodAdvice2 npma = new PostconditionMethodAdvice2(typeDecl, 
						methodDecl, restoreMethod);
				AspectUtil.getInstance().appendPostconditionNewMethodsAroundAdvice(npma.generate(postStmt, defaultPrecondition.toString(),
						...
						defaultEpostCode, defaultOldExprs, defaultOldExprsDecl, defaultPreExprs,
										defaultPreExprsDecl, defaultOldVarsDecl, "clientAwareChecking", 0L, exceptionsInSignalsClauses));// default
			}
			else{
				PreconditionMethodAdvice pma = new PreconditionMethodAdvice(typeDecl, 
						methodDecl, hasDefaultPrecondition, saveMethod);
				if(AspectUtil.hasPrecondition(defaultPrecondition.toString())){
					AspectUtil.getInstance().appendPreconditionNewMethodsAdvice(pma.generate(preStmt, defaultPrecondition.toString(), this
							.getContextValues(defaultPrecondition.toString()),
							defaultPreTokenReference.toString(), "clientAwareChecking", 0L, isEmptySpecCaseWithNonNullPre)); //default
				}
				PostconditionMethodAdvice npma = new PostconditionMethodAdvice(typeDecl, 
						methodDecl, restoreMethod);
				...
				if(AspectUtil.hasAssertion(defaultNPostcondition.toString()) || (defaultEpostCode.size() > 0)){
					JMethodDeclarationType jmdt = npma.generate(postStmt,
							AspectUtil.changeThisOrSuperRefToAdviceRef(defaultNPostcondition.toString(), this.typeDecl), defaultEpostCode, defaultOldExprs,
									defaultOldExprsDecl, defaultPreExprs, defaultPreExprsDecl, defaultOldVarsDecl, "clientAwareChecking", 0L, exceptionsInSignalsClauses); //default
			...
			if (((AspectUtil.getInstance().hasElementsStoredOldExpressions(privateOldExprs)) || (privatePreExprs.size() > 0) || (AspectUtil.getInstance().hasElementsStoredOldExpressions(privateOldVarsDecl)))) {
				PostconditionMethodAdvice2 npma = new PostconditionMethodAdvice2(typeDecl, 
						methodDecl, restoreMethod);
				AspectUtil.getInstance().appendPostconditionNewMethodsAroundAdvice(npma.generate(postStmt, privatePrecondition.toString(),
						...
						privateEpostCode, privateOldExprs, privateOldExprsDecl, privatePreExprs,
										privatePreExprsDecl, privateOldVarsDecl, "clientAwareChecking", ACC_PRIVATE, exceptionsInSignalsClauses));
			}
			else{
				PreconditionMethodAdvice pma = new PreconditionMethodAdvice(typeDecl, 
						methodDecl, hasPrivatePrecondition, saveMethod);
				if(AspectUtil.hasPrecondition(privatePrecondition.toString())){
					AspectUtil.getInstance().appendPreconditionNewMethodsAdvice(pma.generate(preStmt, privatePrecondition.toString(), this
							.getContextValues(privatePrecondition.toString()),
							privatePreTokenReference.toString(), "clientAwareChecking", ACC_PRIVATE, isEmptySpecCaseWithNonNullPre));
				}
				PostconditionMethodAdvice npma = new PostconditionMethodAdvice(typeDecl, 
						methodDecl, restoreMethod);
				...
				if(AspectUtil.hasAssertion(privateNPostcondition.toString()) || (privateEpostCode.size() > 0)){
					JMethodDeclarationType jmdt = npma.generate(postStmt,
							AspectUtil.changeThisOrSuperRefToAdviceRef(privateNPostcondition.toString(), this.typeDecl), privateEpostCode,
									privateOldExprs,
									privateOldExprsDecl, privatePreExprs, privatePreExprsDecl, privateOldVarsDecl, "clientAwareChecking", ACC_PRIVATE, exceptionsInSignalsClauses);
			...
			if(isMethodCrosscutSpecChecking){
				...
				if (((AspectUtil.getInstance().hasElementsStoredOldExpressions(oldExprs) || (preExprs.size() > 0) || (AspectUtil.getInstance().hasElementsStoredOldExpressions(oldVarsDecl))))) {
					PostconditionMethodAdvice2 npma = new PostconditionMethodAdvice2(typeDecl, 
							methodDecl, restoreMethod);
					AspectUtil.getInstance().appendPostconditionNewMethodsAroundAdviceXCS(npma.generate(postStmt, precondition.toString(),
							...
							epostCode, oldExprs, oldExprsDecl, preExprs,
											preExprsDecl, oldVarsDecl, "executionSite", -1, exceptionsInSignalsClauses));
				}
				else{
					PreconditionMethodAdvice pma = new PreconditionMethodAdvice(typeDecl, 
							methodDecl, hasPrecondition, saveMethod);
					if(AspectUtil.hasPrecondition(precondition.toString())){
						AspectUtil.getInstance().appendPreconditionNewMethodsAdviceXCS(pma.generate(preStmt, precondition.toString(), this
								.getContextValues(precondition.toString()),
								preTokenReference.toString(), "executionSite", -1, isEmptySpecCaseWithNonNullPre));
					}
					PostconditionMethodAdvice npma = new PostconditionMethodAdvice(typeDecl, 
							methodDecl, restoreMethod);
					   
					if(AspectUtil.hasAssertion(nPostcondition.toString()) || (epostCode.size() > 0)){
						JMethodDeclarationType jmdt = npma
						.generate(postStmt,
								AspectUtil.changeThisOrSuperRefToAdviceRef(nPostcondition.toString(), this.typeDecl), epostCode,
												oldExprs,
												oldExprsDecl, preExprs, preExprsDecl, oldVarsDecl, "executionSite", -1, exceptionsInSignalsClauses);
				...
				if(Main.aRacOptions.callSiteInstrumentation()){
					if (((AspectUtil.getInstance().hasElementsStoredOldExpressions(oldExprs)) || (preExprs.size() > 0) || (AspectUtil.getInstance().hasElementsStoredOldExpressions(oldVarsDecl)))) {
						PostconditionMethodAdvice2 npma = new PostconditionMethodAdvice2(typeDecl, 
								methodDecl, restoreMethod);
						AspectUtil.getInstance().appendPostconditionNewMethodsAroundAdvice(npma.generate(postStmt, precondition.toString(),
								this.getContextValues(precondition.toString()),
								preTokenReference.toString(),
								AspectUtil.changeThisOrSuperRefToAdviceRef(nPostcondition.toString(), this.typeDecl),
								epostCode, oldExprs, oldExprsDecl, preExprs,
												preExprsDecl, oldVarsDecl, "callSite", -1, exceptionsInSignalsClauses));
					}
					else{
						PreconditionMethodAdvice pma = new PreconditionMethodAdvice(typeDecl, 
								methodDecl, hasPrecondition, saveMethod);
						if(AspectUtil.hasPrecondition(precondition.toString())){
							AspectUtil.getInstance().appendPreconditionNewMethodsAdvice(pma.generate(preStmt, precondition.toString(), this
									.getContextValues(precondition.toString()),
									preTokenReference.toString(), "callSite", -1, isEmptySpecCaseWithNonNullPre));
						}
						PostconditionMethodAdvice npma = new PostconditionMethodAdvice(typeDecl, 
								methodDecl, restoreMethod);
						...
						if(AspectUtil.hasAssertion(nPostcondition.toString()) || (epostCode.size() > 0)){
							JMethodDeclarationType jmdt = npma
									.generate(postStmt,
											AspectUtil.changeThisOrSuperRefToAdviceRef(nPostcondition.toString(), this.typeDecl), epostCode,
															oldExprs,
															oldExprsDecl, preExprs, preExprsDecl, oldVarsDecl, "callSite", -1, exceptionsInSignalsClauses);
...
//	}
	...
	protected List translateLetVarDecl(JmlSpecVarDecl varDecl,
		...
		for (int j = 0; j < varDefs.length; j++) {
			...
			//RacNode node = trans.stmt(true);
			// add code that stores the initializer value to the new field
			//			node = RacParser.parseStatement(
			//			"try {\n" +
			//			"  " + toString(type) + " " + var + " = " + val + ";\n" +
			//			"$0\n" +
			...
			//			"}", 
			//			node.incrIndent(), oldVar);

			//			// piggy-back a new field declaration to the translated code
	 ...
	 * </pre>
	 */
	protected String translateSignalsClause(VarGenerator varGen,
			VarGenerator oldVarGen, JmlSignalsClause sigClause, 
			StringBuffer  xPostconditionForContext, boolean hasTokenReference, String preVar, long visibility, int index, HashMap oldExprs, HashMap oldExprsDecl)
	{
		...
		String ident = sigClause.ident();
		JmlPredicate pred = sigClause.predOrNot();
		String var = varGen.freshPostVar();
		RacContext ctx = RacContext.createPositive();
		boolean isMethodCrosscutSpecChecking = AspectUtil.getInstance().isCrosscutSpecChecking(this.methodDecl);
		String evalErrorDescription = "\"Invalid Expression in \\\"";
		String methodDescription = "";
		if(isMethodCrosscutSpecChecking){
			methodDescription = "by method \"+thisJoinPoint.getSignature().getDeclaringTypeName()+\".\"+thisJoinPoint.getSignature().getName()+\"";
		}
		...
		if(!this.methodDecl.getMethod().owner().getJavaName().contains("$")){
			 context = this.getContextValues(xPostconditionForContext.toString());
		}
...
//		oldExprsDecl.put(index, trans.oldVarDecl());
		...
		// compose error message for later use
		String msg = "";
		...
		if(this.methodDecl.body()!= null){
			if (hasTokenReference) {
				...
				msg = ("line "
					+ +pred.getTokenReference().line() + ", character "
					+ pred.getTokenReference().column() + " ("
					+ this.typeDecl.getCClass().getJavaName() + ".java:"
					+ pred.getTokenReference().line() + ")\"");
			}
		}
       
		if(!msg.equals(NONE) && !msg.equals("\"")){
			result = evalErrorDescription + this.typeDecl.getCClass().getJavaName() + ".java\\\"" + " " + methodDescription + " regarding specifications at \\n" + msg + (context.length()> 0 ? "+\", when \\n\""+context+"+\"" : "+\"") + "\\nCaused by: \"";
			xPostErrMsg = "\"" + methodDescription + " regarding specifications at \\n" + msg + "+\", and \\n\"+"+this.generateXPostErrorMessage(context, trans.getTokenReference());
			xPostErrMsg += "+\"\\n\\t\"+rac$ErrorMsg+rac$e";
		}
		else{
			result = evalErrorDescription + this.typeDecl.getCClass().getJavaName() + ".java\\\"" + " " + methodDescription + "\\nCaused by: \"";
			xPostErrMsg = this.generateXPostErrorMessage(context, trans.getTokenReference());
			xPostErrMsg += "+\"\\n\\t\"+rac$ErrorMsg+rac$e";
		}
		...
		if(isMethodCrosscutSpecChecking){
			endCall = ", runtimeObjectOrStaticType+\".\"+methSig, rac$e);\n";
		}
		else{
			if(this.methodDecl.isStatic() || this.methodDecl.isConstructor()){
				mn = this.methodDecl.getMethod().getJavaName()+AspectUtil.generateMethodParameters(this.methodDecl.parameters(), false).toString();
				endCall = ", \""+ mn+"\", rac$e);\n";
			}
			else{
				mn = "."+this.methodDecl.getMethod().getIdent()+AspectUtil.generateMethodParameters(this.methodDecl.parameters(), false).toString();
				endCall = ", object$rac.getClass().getName() + \"@\" + Integer.toHexString(System.identityHashCode(object$rac))+\""+mn+"\", rac$e);\n";
			}
		}
		
		
		String racErrorMsgUtil = "";
		if(Main.aRacOptions.multipleSpecCaseChecking()){
			if(ident == null){
				racErrorMsgUtil = 
						"\t\t\t   rac$ErrorMsg = \"\";\n"   +
		...
		// happened in a specific project... anyway checks to ensure... [[[hemr]
		if(this.methodDecl.isStatic()){
			if(result.contains("object$rac") || xPostErrMsg.contains("object$rac")){
				result = evalErrorDescription + this.typeDecl.getCClass().getJavaName() + ".java\\\"" + " " + methodDescription + "\\nCaused by: \"";
				xPostErrMsg = this.generateXPostErrorMessage("", trans.getTokenReference());
				xPostErrMsg += "+\"\\n\\t\"+rac$ErrorMsg+rac$e";
			}
		}
		...
		if((preVar != null)){
			code.append("\t\t\t   try{");
			code.append("\t\t\t     "+this.getQuantifierInnerClasses(AspectUtil.changeThisOrSuperRefToAdviceRef(trans.stmtAsString(), typeDecl)));
			code.append("\n\t\t\t     "+var +" = " + AspectUtil.changeThisOrSuperRefToAdviceRef(trans.stmtAsString(), typeDecl) + ";");
			code.append("\n\t\t\t   }");
			code.append("   catch (JMLNonExecutableException rac$nonExec) {\n");
			code.append("\t\t\t     throw new JMLEvaluationError("+result+"+rac$e);\n");
			code.append("\t\t\t   }\n");
			code.append(racErrorMsgUtil);
			code.append("\t\t\t   rac$b = rac$b && " + var + ";\n");
			
			if(!Main.aRacOptions.multipleSpecCaseChecking()){
				code.append("         if(rac$ErrorMsg.indexOf(\"and\") >= 0 ){").append("\n");
				code.append("           rac$ErrorMsg += \" are \";").append("\n");
				code.append("         }").append("\n");
				code.append("         else{").append("\n");
				code.append("           rac$ErrorMsg += \" is \";").append("\n");
				code.append("         }").append("\n");
			}
			
			String xpostKind = (ident != null)? ident : "";
			
			if(Main.aRacOptions.multipleSpecCaseChecking()){
				code.append(space+"JMLChecker.checkXPostMultipleSpecCaseChecking(rac$b,"+ xPostErrMsg+", "+ "\""+xpostKind+"\"" + ", "+Main.aRacOptions.crosscuttingContractSpecifications()+", "+ visibility+endCall);
			}
			else{
				code.append(space+"JMLChecker.checkExceptionalPostcondition(rac$b,"+ xPostErrMsg+", "+ "\""+xpostKind+"\"" + ", "+Main.aRacOptions.crosscuttingContractSpecifications()+", "+ visibility+endCall);
			}
			code.append("\t\t }").append("\n");
		}
		
		
		String translatedSignalsClause = new String(
				// evaludate predicate only if the thrown exception 
				// (rac$e) is of proper type, i.e., this clause's type
				"\n\t\t     if (rac$e instanceof " + type + ") {\n" + 

				// if necessary, initialize exception variable (ident).
				(ident == null ? "\t\t\t   boolean " + var + " = true;\n":
							"\t\t\t   " + type + " " + ident + " = (" + type + ") rac$e;\n" +
							// result var for this xpost
							"\t\t\t   boolean " + var + " = true;\n") +

				// if has no precond, then always eval xpost
				(preVar == null ? "" : code.toString()));

		return translatedSignalsClause;
	}
