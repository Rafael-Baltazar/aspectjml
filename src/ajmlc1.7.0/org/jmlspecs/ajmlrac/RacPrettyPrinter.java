 ...
 * 
 * This file is based on the original $Id: RacPrettyPrinter.java,v 1.77 2007/04/18 23:04:38 leavens Exp $
 * by Yoonsik Cheon
 */
...
/**
 * A visitor class for pretty-printing JML specifications with generated
 * RAC code. Each node of AST is visited and printed to an external file
 * or writer object.
 *
 * <p>
 * <b>Approach to printing JML specifications</b>.
 * The problem is that some JML annotations such as <code>non_null</code>
 * should be printed context-sensitively depending on their surrounding
 * environment. For example, if a <code>non_null</code> annotation appears
 ...
 * </pre>
 *
 * However, if it appears in a model field declaration, it should be 
 * printed without the surrounding JML annotation markers, e.g.
 *
 * <pre>
 *  //@ private model non_null JMLObjectSet elems;
 * </pre>   
 *
 * To handle this, we use a flag variable <code>isAnnotation</code> and
 * the class {@link JmlModifier} to indicate the context of printing. 
 * JML annotation ASTs such as those for <code>non_null</code> annotations 
 ...
 * indicated as such using the method <code>setWithoutMarkers</code>;
 ...
 * is responsible for setting and clearing the <code>isAnnotation</code>
 * flag (e.g., by using <code>mayStartAnnotation</code> and
 * <code>mayEndAnnotation</code>, and directing the return form to the 
 * class <code>JmlModifier</code>.
 * </p>
 *
 * <p>
 * It is the convention that each AST node, if necessary, advances to the
 * next line before printing its code.
 * </p>
 *
 * <p>
 ...
 *
 * @see JmlModifier
 * @see JmlModifier#withoutMarkers()
 * @see JmlModifier#setWithoutMarkers(boolean)
 *
 ...
 *
 */
public class RacPrettyPrinter extends MjcPrettyPrinter
implements RacVisitor {
	 ...
	 *
	 * @param writer the object to which java code be written
	 * @param modUtil the modifier utility to print Java and JML modifiers
	 */
	public RacPrettyPrinter(java.io.Writer writer, JmlModifier modUtil) {
	...
	/**
	 * Constructs a pretty printer object for JML specifications
	 * @param fileName the file into which the code is to be printed
	 * @param modUtil the modifier utility to print Java and JML modifiers
	 */
	public RacPrettyPrinter(String fileName, JmlModifier modUtil) {
		this(new File(fileName), modUtil);
	}

	/**
	 * Constructs a pretty printer object for JML specifications
	 * @param file the file into which the code is to be printed
	 * @param modUtil the modifier utility to print Java and JML modifiers
	 */
	public RacPrettyPrinter(File file, JmlModifier modUtil) {
		super( file, modUtil );
		jmlModUtil = modUtil;
	}
	...
	/**
	 * Constructs a pretty printer object for JML specifications
	 * @param writer the file into which the code is to be printed
	 */
	...
	/**
	 * Prints a compilation unit. This method is overridden here to
	 * not print type declarations in <code>self</code>, the reason
	...
	public void visitCompilationUnit( JCompilationUnit self ) {
		...
		if(Main.aRacOptions.destination() == null){
...
//				}
		...
		for (int i = 0; i < importedPackages.length ; i++) {
			if (!importedPackages[i].getName().equals("java/lang")) {
				if(importedPackages[i].getName().equals("org.jmlspecs.ajmlrac.runtime")){
		...
		for (int i = 0; i < importedClasses.length ; i++) {
			if(!this.isInnerType(importedClasses[i].getName())){
				importedClasses[i].accept(this);
				newLine();
			}
//			else{
//				System.out.println("<excluded>--> "+importedClasses[i].getName());
//			}
		}
	...
	private boolean isInnerType(String importedClass){
		String [] importedClassParsed = importedClass.split("/");
		int count = 0;
		for (int j = 0; j < importedClassParsed.length; j++) {
			if(Character.isUpperCase(importedClassParsed[j].charAt(0))){
				count++;
			}
		}
		if(count > 1){
	...
	public void visitJmlClassDeclaration(JmlClassDeclaration self) {
		...
		String ident = self.getCClass().qualifiedName().replace("/", "_");
...
//		print(modUtil.asString(modifiers).replaceFirst("abstract ", ""));
		//print("class " + ident);

//		if(self.getCClass().isInnerClass()){
//			if(hasFlag(modifiers, ACC_MODEL)){
//				print("static class "+ident.replace("$", "."));
//			}
//			else{
//				print("privileged static aspect "+RacConstants.TN_TEMPORARY_ASPECT_FILE_NAME+ident);	
//			}
//		}
//		else{
//			print("privileged aspect "+RacConstants.TN_TEMPORARY_ASPECT_FILE_NAME+ident);
//		}
		
		boolean canGenerate = true;
		if(hasFlag(modifiers, ACC_STATIC)){
			if(ident.contains("$")){
				canGenerate = false;
			}
		}
		
		if((!self.getCClass().isInnerClass()) && (canGenerate)){
			print(modUtil.asString(modifiers).replaceFirst("abstract ", ""));
			print("privileged aspect "+RacConstants.TN_TEMPORARY_ASPECT_FILE_NAME+ident);
		}
		else if(self.getCClass().isInnerClass()){
			if(hasFlag(modifiers, ACC_MODEL)){
				print("static class "+ident.replace("$", "."));
			}
		}
		...
		/*if (superName != null) {
					...
					//print(" extends " + self.getCClass().getSuperClass().qualifiedName().replace('/', '.'));
...
//		mayEndAnnotation(hasFlag(modifiers, ACC_MODEL));
	...
	{	
		...
		if((!self.getCClass().isInnerClass()) && (canGenerate)){
			...
			print("\n");
			print("\n");
			if(!hasFlag(self.modifiers(), ACC_MODEL)){
				print(" declare precedence: AspectJMLRac_*, *;");
			}
	...
	public void visitJmlInterfaceDeclaration(JmlInterfaceDeclaration self) {
		...
		if((!self.getCClass().isInnerClass()) && (canGenerate)){
			...
			newLine();
			mayStartAnnotation(hasFlag(modifiers, ACC_MODEL));
			print(modUtil.asString(modifiers).replace("abstract ", ""));
	 ...
	 *  As the Mjcvisitor can't deal with \bigint, 
	...
	public void visitArrayAccessExpression(/*@non_null@*/ JArrayAccessExpression self ) {
		...
		String[] strArray = TransUtils.applyBigintToNumber(accessor.getApparentType(), CStdType.Integer);
		String[] strArrayAssertion = TransUtils.createBigintConvertionAssertion(accessor.getApparentType(), CStdType.Integer);
	 ...
	 */
	public void 
	visitCompoundAssignmentExpression(/*@non_null@*/JCompoundAssignmentExpression self) {
		int oper = self.oper();
		JExpression left = self.left();
		JExpression right = self.right();
	...
	public void visitPrefixExpression(/*@non_null@*/ JPrefixExpression self ) {
		...
		if (expr.getApparentType() != JmlStdType.Bigint) {
			...
			//FIXME! if this expression has side-effect sub-expression, there will be an error
			//A statement in a pair of parentheses is an expression, not statement anymore. 
	...
	/**
	 * Prints the given variable definition, <code>self</code>. If the
	 * argument, <code>typeAndMod</code> is true, the type and
	 * modifiers are also printed; otherwise, they are not printed.
	 */
	private void printVariableDefinition(/*@non_null@*/ JVariableDefinition self,
			boolean typeAndMod ) {
		if (typeAndMod) {
			long modifiers = self.modifiers();
	 ...
	 */
	public void 
	visitVariableDeclarationStatement(/*@non_null@*/JVariableDeclarationStatement self) 
	{
	...
	/** Prints a given RacNode. A RacNode is a special AST node for
	 * storing generated runtime assertion check code. It is a
	 * sequence of intermixed <code>RacParser.END_OF_LINE</code>,
	...
	/** Prints multi-lined code with proper line-breaking and
	 * indentation. */
	protected void println(String code) {
		StringTokenizer st 
		= new StringTokenizer(code, NEW_LINE);
	...
	public void visitJmlAccessibleClause( JmlAccessibleClause self ) 
	{
		...
		newLineOffset();
		print(self.isRedundantly() ? 
				"accessible_redundantly " : "accessible ");
	...
	/** Prints a JML assignable clause. If runtime assertion check code
	 * has been generated, it is also printed. */
	public void visitJmlAssertStatement( JmlAssertStatement self ) 
	{
		mayStartAnnotation(true);
		print(self.isRedundantly() ?
				"assert_redundantly " : "assert ");
	...
	/** Prints a JML assume statement. If runtime assertion check code
	 * has been generated, it is also printed. */
	public void visitJmlAssumeStatement( JmlAssumeStatement self ) 
	{
		mayStartAnnotation();
		print(self.isRedundantly() ?
				"assume_redundantly " : "assume ");
		self.predicate().accept(this);
		if (self.hasThrowMessage()) {
			print(" : ");
			self.throwMessage().accept(this);
	...
	/** Prints the given JML field declaration. If this field
	 * declaration has a generated initialzer block (e.g., one for a
	 * ghost field), it is also printed. */
	public void visitJmlFieldDeclaration( JmlFieldDeclaration self ) {
		...
		newLine();
		final boolean startAnnotation = hasFlag(modifiers, ACC_MODEL) 
		|| hasFlag(modifiers, ACC_GHOST);
	...
	public void visitJmlInitiallyVarAssertion( 
	...
	public void visitJmlInvariant( JmlInvariant self ) 
	{
		mayStartAnnotation();
		print(self.isRedundantly() ?
				"invariant_redundantly " : "invariant ");
		self.predicate().accept(this);
		print(";");
		mayEndAnnotation();
	}
	...
	/** Prints a JML loop statement. A JML loop statement is a Java
	 * loop statement with optional loop invariants and variants.
	 */
	...
	public void visitJmlWorkingSpaceClause( JmlWorkingSpaceClause self ) 
	{
		...
		String keyword = self.isRedundantly() ?
	...
	public void visitJmlMonitorsForVarAssertion( 
	...
	public void visitJmlNondetChoiceStatement( 
	...
	public void visitJmlReadableIfVarAssertion( 
			JmlReadableIfVarAssertion self ) {}
	// FIXME - these two assertions are not checked.
	public void visitJmlWritableIfVarAssertion( 
			JmlWritableIfVarAssertion self ) {}

	/** Prints a JML redundant specifications including
	 * <code>implies_that</code> and <code>for_example</code>.
	 */
	...
	public void visitJmlRepresentsDecl( JmlRepresentsDecl self ) 
	{
		...
		mayStartAnnotation();
		print(self.isRedundantly() ?
				"represents_redundantly " : "represents ");
		storeRef.accept(this);
	...
	public void visitJmlSignalsClause( JmlSignalsClause self ) 
	{
		...
		String keyword = self.isRedundantly() ? "signals_redundantly " 
				: "signals ";
	...
	/** Prints a JML store reference keywords such as
	...
	public void visitUnaryPromoteExpression(/*@non_null@*/ JUnaryPromote self ) {
		...
		/* We need to consider two cases in e1 op= e2:
		 * 1) the type of e1 is \bigint
		 * 2) the type of e2 is \bigint and e1 is numeric.
		 * Note that according to the JLS 15.26.2 the above expr is equivalent to
		 * e1 = (type_of_e1)(e1 op e2).
		 *
		 * also, we need to consider cases like:
		 * (\bigint[])null...
		 */
		if ((type == JmlStdType.Bigint) ||
				(type.isNumeric() && expr.getApparentType() == JmlStdType.Bigint)) {
	...
	/**
	 * Prints a character literal. This method is overridden here to
	 * specially handle non-printable characters such as unicode chars.
	 */
	...
	/**
	 * Prints a double literal. This method is overridden here to take
	 * care of special values such as INFINITY, NaN, MIN_VALUE, and
	 * MAX_VALUE.
	 */
	...
	/**
	 * Prints a float literal. This method is overridden here to take
	 * care of special values such as Float.POSTIVE_INFINITY,
	 * Float.NaN, etc.
	 */
	...
	protected String toString(/*@non_null@*/CType type) {
		return ( type instanceof JmlNumericType || 
				TransUtils.isBigintArray(type) )
				? TransUtils.toString(type)
						: super.toString(type.getErasure());
	}
	...
	/** Starts the annotation printing mode if not already in;
	 * also prints the opening annotation marker <code>/*@</code>. 
	 *
	 * <pre><jml>
	...
	/** Stops the annotation printing mode if currently in that mode;
	 * also prints the closing annotation marker <code>@&#042;&#047;</code>. 
	 *
	 * <pre><jml>
	 ...
	 * <code>true </code> and not already in the annotation mode; 
	 * also prints the opening annotation marker <code>/*@</code>. 
	 *
	 * <pre><jml>
	 * modifies annotationDepth, inAnnotation, jmlModUtil, atMarkerPos;
	 ...
	 * <code>true </code> and currently in the annotation mode;
	 * also prints the closing annotation marker <code>@&#042;&#047;</code>. 
	 *
	 * <pre><jml>
	 * modifies annotationDepth, inAnnotation, jmlModUtil;
	...
	/** Prints a new line marker. If in the annotation mode, also print
	 * the annotation marker at the appropriate position.
	 */
	...
	/** Prints a new line marker. If in the annotation mode, also print
	 * the annotation marker at the appropriate position and advance to
	 * the distance of the current offset.
	 */
