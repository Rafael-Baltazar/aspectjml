 ...
 *
 * $Id: InvariantMethodAdviceAsPreconditionMethod.java,v 1.0 2009/01/21 22:12:45 henriquerebelo Exp $
 */
 ...
 */
public class InvariantMethodAdviceAsPreconditionMethod extends InvariantLikeMethod{
	// ----------------------------------------------------------------------
	// CONSTRUCTORS
	// ----------------------------------------------------------------------
	 ...
	 */
	public InvariantMethodAdviceAsPreconditionMethod(boolean isStatic, JmlTypeDeclaration typeDecl, VarGenerator varGen)
	{ 
	...
	protected StringBuffer buildBeforeAdvice() 
	{
		...
		if (!this.getInvariantsTokenReference(false).equals("")){
			...
			errorMsgForInstInv += (!this.getContextValuesForDefaultInvariant(false, varGen).equals("")? "+\"\\n"+this.getContextValuesForDefaultInvariant(false, varGen)+"\"":"");
		...
		if((this.hasInstInv) || (this.hasStatInv)){
			...
			if( (this.hasInstInv) && (this.hasStatInv)){	
				...
				code.append(")").append(" :");
				code.append("\n");
				code.append("   (execution(!static * "+classQualifiedName+"+.*(..))").append(adviceexecution).append(")");
				code.append(" && ");
				code.append("\n");	
