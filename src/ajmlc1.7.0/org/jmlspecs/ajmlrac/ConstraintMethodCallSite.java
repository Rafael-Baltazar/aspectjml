...
public class ConstraintMethodCallSite extends AssertionMethod {
	 ...
	 *                 are to be generated.
	 */
	protected ConstraintMethodCallSite(boolean isStatic, 
	...
	private void generateAroundAdviceForUniversalConstraintChecking(
		...
		if(forStatic){
			...
			code.append("   (call(!static * *(..))").append(" && \n");	
			code.append("   !call(void finalize() throws Throwable)").append(adviceexecution).append(")");
			code.append(" && ");
			code.append("\n");	
			if (!Main.aRacOptions.ajWeaver().startsWith("abc") && Float.parseFloat(Main.aRacOptions.source()) > 1.4){
				code.append("   !@annotation(JMLHelper)");
...
//		}
	...
	private void generateAroundAdviceForSpecificConstraintChecking(
		...
		if(forStatic){
			...
			for (int i = 0; i < methodNames.length; i++) {
				if((i == 0) && (methodNames.length > 1)){
					  code.append((methodNames[i].contains("new")? "   (call("+methodNames[i]+")" : "   (call(* "+methodNames[i]+")"));
				}	
				else if((i == 0) && (methodNames.length == 1)){				
				  code.append((methodNames[i].contains("new")? "   call("+methodNames[i]+")" : "   call(* "+methodNames[i]+")"));
				}
